...
function showWeaterReport(weather) {
    ...
    if(city_code==='400'){ 
        swal("Empty Input", "Please enter your location", "error");
        reset();
    }else if(city_code==='404'){
        swal("Wrong Input", "entered city didn't match", "warning");
        reset();
    }
    else{
    ...
    <div class="weather-status">
        ...
        <div class="min-max" id="min-max">${Math.floor(weather.main.temp_min)}&deg;C (Min) <br>  ${Math.ceil(weather.main.temp_max)}&deg;C (Max) </div>
        <div id="updated_on">Updated as of ${getTime(todayDate)}</div>
    </div>
    <hr>
    <div class="day-details">
        <div class="basic">Humidity ${weather.main.humidity}% | Wind ${weather.wind.speed} KMPH</div>
    </div>
    `;
